:description: This chapter presents a visual overview of the UI of ONgDB Browser.
...
* *Connected as* -- Shows the current user and their roles and the Geequel command to disconnect from the server.
...
Use the *Add empty favorite* option to open an empty script in the Geequel editor, then write the query that you want to bookmark.
...
The first line of the Geequel query or command is used to name your favorite.
...
Project files allow you to save queries, guides, and other scripts, as Geequel files.
...
// Geequel - https://github.com/graphfoundation/ongdb-browser/blob/master/src/browser/documentation/guides/cypher.tsx
...
| `:help geequel`
| Provide a list of the most useful Geequel commands, with an overview and a quick way to find help.

| `:help commands`
| List all available `:help` commands.

| `:help keys`
| List some of the Geequel editor keyboard shortcuts. +
To access the list of all available editor keybindings, press F1 in the Geequel editor.
...
The Geequel editor is the primary interface for entering and running Geequel queries and commands.

image:screen_editor.png[]

[NOTE]
====
From ONgDB Browser 2.0, the Geequel editor is based on link:https://microsoft.github.io/monaco-editor/[Monaco Editor^], the code editor that powers link:https://github.com/Microsoft/vscode[Visual Studio Code (VS Code)^].
...
Some of the Geequel editor features include:
...
| Move focus to the Geequel editor.
| */*
| */*

| Toggle the Geequel editor to full screen.
| *ESC*
| *ESC*
|===
...
Each reusable frame maintains its own local history of commands and updates the main one in the Geequel editor, should you need to instantiate a new result frame from there.
