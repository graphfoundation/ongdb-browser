:description: Adjust ONgDB Browser settings.
...
| `codeFontLigatures`
| `true`
| Allow font ligatures for the Geequel editor bar and Geequel snippets.
| yes

| `useBoltRouting`
| `false`
| Whether to use Bolt routing.
The Bolt+routing feature on ONgDB drivers discovers a cluster member (drivers have a load-balancing feature).
| no
...
| `connectionTimeout`
| `30000`
| The timeout in ms when establishing a connection to ONgDB.
| yes
...
To change the configuration settings for all users of ONgDB Browser, modify the _ongdb.conf_ file, or if using link:[ONgDB Desktop], navigate to the DBMS which settings you want to update, click the ellipsis dropdown menu, and select *Settings*.

For example, to change the `maxFrames` from its default of `50` to `10` and the theme to `outline`, add the following line to _ongdb.conf_:
...
This changes the `maxFrames` from its default of `50` to `10` and then restarts ONgDB.
