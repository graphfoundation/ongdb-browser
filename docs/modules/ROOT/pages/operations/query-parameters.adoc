:description: Geequel query parameters.
...
ONgDB Browser supports querying based on parameters.
It allows the Geequel query planner to re-use your queries instead of parse and build new execution plans.
...
[TIP]
====
For more details on the Geequel parameters, see link:https://docs.graphfoundation.org/geequel-manual/{neo4j-version}/syntax/parameters/[Geequel Manual v.{neo4j-version} - Parameters^].
====
...
The right hand side of `=>` is sent to the server and evaluated as Geequel with an implicit `RETURN` in front.
This gives better type safety since some types (especially numbers) in JavaScript are hard to match with ONgDB's type system.
...
.Geequel query example with a parameter
=====
...
In ONgDB Browser, the current state of multi-statement is to set up your environment with multiple statements so that you can execute queries and examine the results, one by one.
...
It is possible to save the result from a Geequel query to a parameter.
...
You can clear all currently set parameters from ONgDB Browser by running:
...
Since the result frame is reusable, once you have set your parameter(s), you can run the same Geequel query again without having to re-enter it.
...
//Save the favorites to persist on the disk if you are using ONgDB Desktop.
